...
function buildModel(product) {
  ...
  let model;
  if (product.id.includes('earring')) { model = buildEarrings(product); } else { model = buildNecklace(product); }
...
function getProductThumbnail(product) {
  if (product.id.includes('necklace') || product.id === 'choker') {
    return `<svg viewBox="0 0 50 50" fill="none"><path d="M10 15 Q25 35 40 15" stroke="${product.color}" stroke-width="2" fill="none"/><circle cx="25" cy="32" r="3" fill="${product.color}"/></svg>`;
  }
  return `<svg viewBox="0 0 50 50" fill="none"><circle cx="16" cy="25" r="5" fill="${product.color}" opacity="0.7"/><circle cx="34" cy="25" r="5" fill="${product.color}" opacity="0.7"/></svg>`;
}
